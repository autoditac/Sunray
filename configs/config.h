...
//#define BLE_NAME      "Alfred" // Bluetooth Low Energy (BLE) name to advertise (commented out: BLE disabled)
...
//#define ENABLE_LIFT_DETECTION  1  // disabled: lift sensor causes frequent false triggers on uneven terrain
// should the lift sensor be used for obstacle avoidance (if not, mower will simply turn off cutter motor if lifted)
//#define LIFT_OBSTACLE_AVOIDANCE 1  // disabled: see above
...
// confirmation (rules out slipping).  See docs/Alfred/2026-04-21-wheel-stall-detection.
...
// Minimum battery voltage required for the current/slope undock criteria to be valid.
// Background: if dock contact resistance is high or the charger output sags, chargingCurrent
// can read below BAT_FULL_CURRENT and batteryVoltageSlope can sit near zero even though the
// pack is far from full (observed on batman 2026-04-29: undock at 27.5 V because diffV between
// charger and battery was only ~0.5 V → I_charge stayed at 60–140 mA instead of >1 A CC).
// Voltage-based criteria (BAT_FULL_VOLTAGE) and the BMS hardware cutoff are unaffected.
#define BAT_FULL_MIN_VOLTAGE 30.0
...
#define GPS_SPEED_DETECTION false  // disabled: VELNED groundSpeed unreliable at 0.1 m/s, use GPS_MOTION_DETECTION instead
...
// Receiver configuration is persisted by alfred-ansible (`--tags gps`).
// Sunray may warm-restart the GNSS engine for recovery, but it must not rewrite
// the receiver profile at runtime.
#define GPS_REBOOT_RECOVERY  true   // allow GNSS warm restart during GPS recovery
//#define GPS_REBOOT_RECOVERY   false  // do not warm-restart the GNSS receiver

//#define GPS_CONFIG   true   // configure GPS receiver at runtime
#define GPS_CONFIG   false    // receiver profile is managed by alfred-ansible
...
//#define BUZZER_ENABLE 1 // disabled: beeps not wanted
